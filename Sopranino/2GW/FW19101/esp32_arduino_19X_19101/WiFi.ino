...
void connectToWifi() {
  ...
  strlcpy(Tstick.lastConnectedNetwork, "guitarhub", sizeof(Tstick.lastConnectedNetwork));
  //password of your WPA Network
  strlcpy(Tstick.lastStoredPsk, "mappings", sizeof(Tstick.lastStoredPsk));
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    if (WiFi.status() == WL_CONNECTED) {
      Serial.print("\nThe T-Stick is connected to "); Serial.print(Tstick.lastConnectedNetwork); Serial.println(" network");
      Serial.print("IP address: "); Serial.println(WiFi.localIP());
      }
    else {Serial.println("Failed to connect to the saved network. Enter setup to connect to a different network.");}
    }
  else {
    Serial.print("The T-Stick is connected to "); Serial.print(Tstick.lastConnectedNetwork); Serial.println(" network");
    Serial.print("IP address: "); Serial.println(WiFi.localIP());
    }
}
...
void Wifimanager_portal(char *portal_name, char *portal_password) {
  ...
  if (strcmp(wifimanager_APpasswd.getValue(), wifimanager_APpasswdValidate.getValue()) == 0 ) {
    ...
    Serial.println("T-Stick passwords don't match! Not saving new password.");
    }
