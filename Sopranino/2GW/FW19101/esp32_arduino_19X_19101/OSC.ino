...
byte receiveOSC() {
  ...
  if (size > 0) {
    ...
    if (!msgReceive.hasError()) {
      ...
      msgReceive.dispatch("/state/FSRcalibration", receiveFSRcalibration); // receive FSRcalibration and FSRcalibrationValues[2] (doesn't save to JSON)
    } else {
...
void sendInfo(OSCMessage &msg) {
  ...
  OSCMessage msgInfo("/info");
    ...
    msgInfo.add(Tstick.FSRcalibration);
    msgInfo.add(Tstick.FSRcalibrationValues[0]);
    msgInfo.add(Tstick.FSRcalibrationValues[1]);
